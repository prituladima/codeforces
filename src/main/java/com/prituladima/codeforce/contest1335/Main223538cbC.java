...
public class Main223538cbC {
    ...
    private void solve() {
        ...
        for (int i = 0; i < n; i++) {
            counter.merge(a[i], 1, Integer::sum);
        }

        int maxAns = 1;

        for(int skill: counter.keySet()){
            //1.
            maxAns = max(maxAns, min(counter.get(skill), counter.size() - 1));

            //2.
            maxAns = max(maxAns, min(counter.get(skill) - 1, counter.size()));
